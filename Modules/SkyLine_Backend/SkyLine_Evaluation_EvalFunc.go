...
func evalPrefixExpression(operator string, right Object) Object {
	...
	default:
		return newError("unknown operator: %s%s", operator, right.Type_Object())
	}
}
...
func evalInfixExpression(operator string, left, right Object) Object {
	...
	case left.Type_Object() != right.Type_Object():
		return newError("type mismatch: %s %s %s", left.Type_Object(), operator, right.Type_Object())
	default:
		return newError("unknown operator: %s %s %s", left.Type_Object(), operator, right.Type_Object())
	}
}
...
func extendFunctionEnv(fn *Function, args []Object) Environment {
	...
	for i, param := range fn.Parameters {
		...
		defer func() {
			if x := recover(); x != nil {
				fmt.Print("\n\n")
				fmt.Println("ERROR: TRACE --------------> FUNCTION SEEN BELOW \n TRACE : \n", fn.Inspect(), " \nTRACE : DOES NOT HAVE PROPER PARAMS DURING CALL! ")
				fmt.Println("TRACE : Function params -> ", fn.Parameters)
				fmt.Println("TRACE : Function Length -> ", len(fn.Parameters))
				os.Exit(0)
			}
		}()
	}
...
func applyFunction(fn Object, args []Object) Object {
	...
	default:
		...
		return newError("not a function: %s", fn.Type_Object())
	}
}
...
func evalIndexExpression(left, index Object) Object {
	...
	default:
		return newError("index operator not supported: %s", left.Type_Object())
	}
}
...
func evalHashLiteral(node *HashLiteral, Env Environment) Object {
	...
	for keyNode, valueNode := range node.Pairs {
		...
		if !ok {
			return newError("unusable as hash key: %s", key.Type_Object())
		}
...
func evalHashIndexExpression(left, index Object) Object {
	...
	if !ok {
		return newError("unusable as hash key: %s", index.Type_Object())
	}
